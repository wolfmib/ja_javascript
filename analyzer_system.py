...
class image_process_agent():
    def __init__(self):
        self.name = 'image_process_agent'
        self.function_list = ['rotate','blur','cor']


    # ----- image process agent reference code ------
        # -  Rotate
            #    for v in range(700):
            #
            #        each_deg = random.randrange(-350, 350)
            #        each_deg = each_deg / 10  # -35  ~ 35 deg
            #        tem_image = image.rotate(each_deg)
            #        # ./train_models/dix_dir/Train/dix/1.jpg
            #        __saved_path = pre_path + "/%s/%d.jpg" % (norm, v)
            #        tem_image.save(__saved_path, format='JPEG')
            #
        # -  Blur
            #    for v in range(700, 900):
            #        each_deg = random.randrange(-100, 100)
            #        each_deg = each_deg / 10
            #
            #        tem_image = image.filter(ImageFilter.BLUR)
            #        __saved_path = pre_path + "/%s/%d.jpg" % (norm, v)
            #        tem_image.save(__saved_path, format='JPEG')
            #
        # -  Cor
            ...
            #        each_deg = each_deg / 10
            #
            #        main_value = random.randrange(90, 100)   # 100
            #        sub_value = random.randrange(
            #            int(main_value-10), int(main_value-5))  # 90 * V / 100
            #        left = w / main_value
            #        top = h / main_value
            #        right = sub_value * w / main_value
            #        bottom = sub_value * h / main_value
            #
            #        tem_image = image.crop((left, top, right, bottom))
            #        __saved_path = pre_path + "/%s/%d.jpg" % (norm, v)
            #        tem_image.save(__saved_path, format='JPEG')
    def rotate(self,input_image, input_path, input_format='JPEG'):
        each_deg = random.randrange(-350, 350)
        each_deg = each_deg / 10  # -35  ~ 35 deg
        tem_image = input_image.rotate(each_deg)
        tem_image.save(input_path, format=input_format)
    def blur(self,input_image, input_path, input_format='JPEG'):
        each_deg = random.randrange(-100,100)
        each_deg = each_deg / 10

        tem_image    = input_image.filter(ImageFilter.BLUR)
        tem_image.save(input_path,format=input_format)
    def cor(self,input_image,input_path, input_format='JPEG'):
        ...
        # Cor
        main_value = random.randrange(90, 100)   # 100
        sub_value = random.randrange(int(main_value-10), int(main_value-5))  # 90 * V / 100
        left = w / main_value
        top = h / main_value
        right = sub_value * w / main_value
        bottom = sub_value * h / main_value

        tem_image = input_image.crop((left,top,right,bottom))
        tem_image.save(input_path,format=input_format)

    # ----- image cutter reference code ---------------
        # referenced code: 97_project/SLOT_5X3_TOTAL_15_CUTTER/97_automation_cutter_v01.py
        # description:
            # - # Crop Code:  # from PIL import Image:
                # Col Moving Direction: C'est Height, index = i
                # Row Moving Direction: C'est width,  index = j
                # Je vais faire les images  col_row comme ca:
                    #        col_1, col_2, col_3, col_4, col_5
                    # row_1   1_1 ,  2_1  , 3_1  , 4_1,   5_1
                    # row_2   1_2,   2_2,   3_2  , 4_2 ,  5_2
                    # row_3   1_3,   2_3,   3_3 ,  4_3  , 5_3
            # nb_of_cols = 5 , nb_of_rows = 3 , input_dir_path = 'work_tem/run_%d'
    def crop(self,input_dir_path,input_image_path, nb_of_cols, nb_of_rows):
        im = Image.open(input_image_path)
        imgwidth, imgheight = im.size

        col_index = 1 # 1-5 par example
        row_index = 1 # 1-3 par example

        petit_width  = imgwidth / nb_of_cols
        petit_height = imgheight / nb_of_rows

        for i in range(0, int(imgheight-petit_height)+1, int(petit_height)):

            # Initial row_index pour tout le col
            col_index = 1
            for j in range(0, int(imgwidth-petit_width)+1, int(petit_width)):
                box = (j, i, j+ petit_width, i + petit_height)
                a = im.crop(box)
                try:
                    __saved_path = input_dir_path + "/" + "%d_%d.jpg"%(col_index,row_index)
                    a.save(__saved_path,format='JPEG')
                except:
                    print("[Error]: Save failure with the path = %s"%__saved_path)
    ...
    # ----- count how many phots under spesific folder ---- 
        # The idea is from train_all_models_by_pick_best.py
        # Je vais train tout le models, mais the sample_number sont different par tout le class
            # code pour obtenir Train/class, not_class,  Validaton/class, not class:
                # --------------------------------------------------------
                #          _cmd = "ls train_models/%s_dir/Train/%s | grep 'jpg' | wc -l"%(any_norm,any_norm)
                #          res_1 = int(os.popen(_cmd).read())
                #          print("Check here: %d" % res_1)
                #          _cmd = "ls train_models/%s_dir/Train/not_%s | grep 'jpg' | wc -l"%(any_norm,any_norm)
                #          res_2 = int(os.popen(_cmd).read())
                #          print("Check here: %d"%res_2)
                #          _cmd = "ls train_models/%s_dir/Validation/%s | grep 'jpg' | wc -l"%(any_norm,any_norm)
                #          res_3 = int(os.popen(_cmd).read())
                #          print("Check here: %d"%res_3)
                #          _cmd = "ls train_models/%s_dir/Validation/not_%s | grep 'jpg' | wc -l"%(any_norm,any_norm)
                #          res_4 = int(os.popen(_cmd).read())
                #          print("Check here: %d"%res_4)
                #          
                #          nb_image_in_train      = res_1 + res_2
                #          nb_image_in_validation = res_3 + res_4
                #          print(" Total Train number is %d"%nb_image_in_train)
                #          print(" Total Validation number is %d"%nb_image_in_validation)
    def obtenir_images_by_folder_avec_os_mac(self,input_folder,key_word='jpg'):
        __cmd = "ls %s | grep '%s' | wc -l"%(input_folder,key_word)
        res_pour_numbers   = int(os.popen(__cmd).read())
        return res_pour_numbers
...
if __name__ == '__main__':
    ...
    for any_language in translator.language_dict:
        ...
        _info = translator.translate_par_les_mots_et_dest(my_words,any_language)
        print(_info)
        print("--------------------------------------------------------------------------------------------\n\n")
